...
class FormularioPacienteEditar(forms.ModelForm):
    class Meta:
        model = Paciente
    ...
    def clean_dpi(self):
        ...
        if Paciente.objects.filter(dpi=dpi).exclude(pk=instance.pk).exists():
            raise forms.ValidationError("Este DPI ya está en uso por otro médico.")
        return dpi
...
class FormularioMedicoEditar(forms.ModelForm):
    class Meta:
        model = Medico
    ...
    def clean_numero_colegiado(self):
        numero_colegiado = self.cleaned_data.get('numero_colegiado')
        instance = self.instance
        if Medico.objects.filter(numero_colegiado=numero_colegiado).exclude(pk=instance.pk).exists():
            raise forms.ValidationError("Este número de colegiado ya está en uso por otro médico.")
        return numero_colegiado
